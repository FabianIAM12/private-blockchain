...
class Blockchain {
    ...
    async initializeChain() {
        if( this.height === -1){
            let block = new BlockClass.Block({data: GENESIS});
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.map(block => {
                const selectedBlock = block.getBData();

                if (selectedBlock && selectedBlock.data) {
                    const message = selectedBlock.data;
                    const walletAddress = message.split(':')[0]

                    if (walletAddress === address) {
                        stars.push( selectedBlock.star);
                    }
                }
            })
            resolve(stars);
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i < self.chain.length; i++) {
                if (self.chain[i] && self.chain[i].getBData()) {
